    ...
    if (stored) {
      try {
        ...
        if (now >= resetDate) {
          ...
          setUsageData({
            fileScans: parsedData.fileScans ?? 0,
            urlScans: parsedData.urlScans ?? 0,
            resetDate: parsedData.resetDate ?? getNextResetDate(),
            monthStart: parsedData.monthStart ?? getCurrentMonthStart(),
          });
        }
      } catch (error) {
